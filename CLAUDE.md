...
| `tests/`, `package.json`, `package-lock.json` | Headless regression harness (`npm test`), dev-only; the lockfile pins `playwright-core` so the suite is reproducible. `tests/run.js` is the browser suite; `tests/reachability.js` and `tests/coverage.js` are static specs that need no browser and run **first**; `tests/probe.js` drives the real app, clicking every control on every route (§12.5); `tests/playthrough.mjs` plays a whole solo session (`npm run playtest`, §12.7) |
| `.claude/agents/solo-playtester.md` | The playtester agent: audits a session (mode 1) or plays and writes one (mode 2) |
| `tests/play.mjs` | Stepwise driver — one beat per invocation, so a caller with judgement can decide and narrate (§12.8) |
| — | Commands: `npm test` (fast, ~50s) · `npm run probe` (adds the click sweep) · `npm run playtest` (plays a session) · `npm run guide` |
...
## 12.7 The playthrough — can a person actually play a session?

`tests/playthrough.mjs` (`npm run playtest`) plays **one complete solo session** against the real
UI: sitting down cold → quick-building a hero → switching on Crisis Mode → alert → event check →
engaging → three beats of play → a social scene → resolving → ending → coming back next week. It
finds controls by their visible label, presses them, answers whatever dialog appears the way a
player would (taking the highlighted default), and writes a transcript.

Its finding class is the **stall**: a beat where the app offered nothing that moves play forward.
Stalls are invisible to every other spec — reachability sees the control existing *somewhere*,
coverage sees the rule implemented — and only appear when someone tries to play. `Math.random` is
seeded so a session is reproducible and a regression is a real change rather than variance; run
several seeds, because a path that works on seed 1 can dead-end on seed 4.

A clean run states what it does **not** cover: combat blow by blow, multi-crisis campaigns, karma
spending between sessions, group play, and whether the fiction the oracles produced was any *good*
— only that play always had somewhere to go.

`.claude/agents/solo-playtester.md` is the agent that runs it and reads the transcript critically.

**The seed varies the path, not just the dice.** The agent's own first report caught that it did
not: with every chooser answered by taking the first option, seeds changed only flavour text and
numbers, so all of them walked the identical session — no timer ever started, karma never left 0,
and combat was never reached, making "run several seeds" unbuyable advice. The harness now has its
own PRNG, deliberately separate from the page's so a dialog choice never shifts the app's dice, and
it types into empty prompt fields (several flows discard silently on an empty value). It also
presses the Crises panel's own **Engage** rather than the next-step card's *"Engage a crisis"*,
which only scrolls — that one control was why no session had ever run a timer.

**The probe is opt-in.** Clicking every control on every route takes minutes, which is too slow to
sit in front of every commit — and a suite people skip catches nothing. `npm test` is ~50s and
**says** the sweep was skipped rather than skipping silently (§12.6); `npm run probe` includes it.
CI should run the probe; a developer mid-change usually should not.

## 12.8 Playing, as opposed to driving

`tests/playthrough.mjs` proves the machinery holds together. It cannot prove the game is worth
playing, because it decides nothing for a reason and writes no prose — its journal is a log.

`tests/play.mjs` is the other half: **one beat per invocation**, printing the hero, the crisis, the
live timers, the recent journal and either the open dialog's options or the screen's controls, then
waiting. A caller with judgement reads the fiction, decides what the hero does *because of* it, and
writes the entry. Steps run as a sequence in a single invocation (`do … choose … write …`) because
a dialog cannot survive the page reload between invocations; the campaign persists in
`tests/.play-state.json`, so a session continues across commands and can be picked up later.

This is what makes the journal a record rather than a log: the app's engines still roll every die
and generate every oracle result, and the player supplies the only thing they cannot — what it
meant.
...
|---|---|---|---|---|
| 2026-09-06 | **A thorough playthrough, and the bug that had silently locked solo karma since it shipped.** The playtester played a 127-entry sitting across two crises, deliberately driving at everything the first session never touched. (1) **`Head home` was unreachable, so every karma a solo player earned was permanently unspendable.** `resolveCrisis()` increments `state.resolved` and clears `state.alert` and `state.eventChecks` **in the same call**, while `currentStep()` gated step 6 on a live alert — so the state step 6 required could never exist. Head home is the only thing that sets `spendUnlocked`, and under §3.20 objective timers are the *entire* solo karma economy, so the whole advancement loop was dead. Worse, *End the session*'s own dialog told the player to *"use Head home instead"* — a button that could not appear. Step 6 is now tested **before** the no-alert case. (2) **A successful purchase was journaled as `Failure`** — the automatic branch (Resources above Cost, no roll) logs an empty pool, and `describeOutcome` read 0 sixes as failure; it now names the automatic case. Same class as the v55 lying-journal fix. (3) **Help (§3.1) was implemented and unreachable** — `buildPool` has always accepted a `help` modifier and no caller ever passed one; `rollAttribute` now asks, but **only when someone could actually be helping** (an ally on the board or a solo ally group with people left), because prompting before every solitary roll would be worse than the missing rule. (4) A push refused at 0 Resolve silently hid its control; it now names the reason (§12.6). | Found by playing. | `npm test` — 492 passed, 0 failed; 5 new checks. **The root cause of #1 was in the test, not just the code:** the old check built `{alert:"x", eventChecks:1, resolved:1}` by hand and asserted `currentStep` returned 5 — a state the app can never produce. It is now built from what `resolveCrisis` actually leaves behind, plus the two neighbouring cases (a social scene still owed, a second crisis picked up before going home). This is the same failure as the A5 counterattack a day earlier: **asserting a function against hand-made state proves nothing about whether the path exists.** | v57 |
| 2026-09-05 | **Three defects found by actually playing a fight.** The playtester played a full session in mode 2 — a rolled hero, Daresay, losing a fight to an armoured brawler — and hit three things no spec had. (1) **A hero on the board had two health values and only one direction ever synced.** `applyAttackDamage` refreshed the card from the character after damage, but nothing refreshed it after healing, so a Rally on the Sheet left the board still believing the hero was broken: their **Attack button disappeared** and the target picker offered stale numbers. `syncHeroCombatants()` now runs on every combat render — the character is authoritative and the card is a view of it. (2) **Damage from a fully-resolved attack could silently vanish.** *Apply damage* sat beside a button labelled **Done**, which reads like a confirm; dismissing left the roll in the journal saying the blow landed while the board showed the target untouched. The opt-out is now named *Don't apply it*, the primary says *Apply N damage*, and dismissing an unapplied hit warns that the target is untouched. (3) **The counterattack was described and could not be performed.** A block cancelling more 6s than the attack threw printed *"Counterattack! It hits automatically…"* and offered no control to deal it — audit A5's arithmetic was right and tested, but the test called `resolveBlock` with fake numbers and never opened the dialog, so the missing *path* was invisible. The result dialog now offers **Counterattack for N**. | Found by playing, not by auditing. | `npm test` — 487 passed, 0 failed; 4 new checks: the card follows the character in **both** directions, a surplus block reports a counterattack while an exact block does not, and the dialog both offers the counterattack and names the opt-out. Two self-inflicted errors on the way: the new check first used a fixed 4200-byte slice of `showAttack` as its marker and failed when the warning sat 158 bytes past the window (the brittle-marker trap `docs/COVERAGE.md` warns about), and it called `localStorage.clear()`, wiping the action scene a later test reads. | v56 |
| 2026-09-04 | **The journal told a lie, and the playtester learned to play.** Two things. (1) **A false line in the permanent record.** Ending a session after resolving the crisis wrote *"Session closed: paused mid-crisis"* — the earlier fix had renamed the button and its dialog title but not the text they write, so the one artefact a player keeps stated the opposite of what happened. `stopForTonight` now branches on whether the crisis was actually won, across all four things it emits: the dialog's lede, its detail line, its confirm label (**End it here** vs *Stop here*) and both journal lines. (2) **`tests/play.mjs` — a stepwise driver, so the agent can play rather than drive.** The batch playthrough presses controls on a fixed spine and picks at random; it proves the machinery holds and writes not a word of prose, so its journal is a log. The new driver runs **one beat per invocation** — printing the hero, the crisis, the timers, the recent journal and either the open dialog's options or the screen's controls — and waits for a caller with judgement to decide and narrate. Steps run as a sequence in one invocation (`do … choose … write …`) because a modal cannot survive the page reload between invocations; the campaign persists in `tests/.play-state.json`. The agent gained a second mode documented against it. | Requested. | Driven by hand end to end: a rolled hero, an alert (*Business organization: Fire! Flames burn out of control (Laboratory), Complication: Crowded*), a written entry, and an event check rolling 12 — an opportunity event — all landing in the journal in order. `npm test` — 483 passed, 0 failed. | v55 |
| 2026-09-03 | **The playtester's first real finding was about the playtester.** Asked to fix what the agent found. Its verdict was clean — three seeds, 0 stalls — but it reported that all seeds took the *mechanically identical* path: the seed changed the alert's flavour text and the event-check numbers, never which branch the session took, because the harness answered every chooser by taking the first option. So **no timer was ever started, karma never left 0, and combat was never reached** — the sessions exercised the spine and none of the engine, and the agent file's own "run several seeds" advice was unbuyable. Four fixes, all in the harness rather than the app. (1) A harness-side PRNG, deliberately **separate** from the page's seeded `Math.random`, picks among `.choice` lists, so a dialog choice never shifts the app's dice and a seed still reproduces exactly. (2) The harness pressed *"Engage a crisis"* — the next-step card's control, which only **scrolls** to the Crises panel — and stopped; it now also presses that panel's own **Engage**, which is what actually starts a timer. (3) It never typed: `addObjective` is a prompt and `if (!name) return` discarded it, so every empty field is now filled before confirming. (4) The step-4 control's *label* is "Say what your hero just did" while "What did your hero just do?" is the *dialog title*; matching only the title fell through to a loose `Check\|Roll` fallback that hit the attribute-guide reference panel instead of advancing play. Sessions now set an objective, roll its progress, run a live crisis timer and see the crisis level move. | Requested: fix what it finds. | Seeds 1, 5 and 11 each play 12 beats with 0 stalls, 0 problems, 0 console errors and 16-18 journal entries — against 10 beats and 6 entries before, on a single repeated path. `npm test` — 483 passed, 0 failed. | — (no shipped file changed; harness and agent only) |
| 2026-09-02 | **A playtester agent, and the two bugs it found in its first session.** Asked for an agent that simulates playing the game completely, solo. `tests/playthrough.mjs` plays one whole session against the real UI — finding controls by their visible label, pressing them, answering dialogs the way a player would — and reports **stalls**: a beat where the app offered nothing that moves play forward. `.claude/agents/solo-playtester.md` is the agent that runs it across seeds and reads the transcript. It found two real defects on its very first run, both invisible to every existing spec. (1) **A solo session never opened a journal session** — `Journal.startSession` was exported and called from nowhere in `solo.js`, so an entire evening's entries landed unfiled: the verdict line read *4 entries across 0 sessions*. Generating an alert now opens the sitting, because an alert IS the start of one. (2) **The ending vanished at the moment you won.** *Stop for tonight* was gated on `alert || timers`, and resolving a crisis clears both — so at the exact beat a player finishes the thing they came to do, neither ending was on screen. New `inSession()` is the single honest definition of "a sitting is in progress" (alert, timers, crises, awaiting a social scene, anything resolved, or any event check), used by both the ending control and the Home card; the control renames itself to **End the session** once the crisis is resolved, and its dialog now agrees rather than still saying *Stop for tonight*. | Requested. | Four seeds play start to finish with 0 stalls, 0 problems, 0 console errors. `npm test` — 483 passed, 0 failed in ~50s; 3 new checks pinning the fixes: an alert opens a journal session, and `inSession` still reports true for a won-but-unclosed sitting while staying false on a pristine board. One finding was my harness rather than the app — it looked only for "Stop for tonight" when the app was correctly offering "End the session". | v54 |
| 2026-09-01 | **One spine, and an always-present "how do I play?".** Reported: still unclear how to start, sustain and end. Root cause was a genuine contradiction — `sessionStage()` had no idea Crisis Mode existed, so the **Sheet** offered the table lifecycle ("Start session" → "Start action scene") while the Solo tab offered the Ch.9 loop. Home had already been fixed separately; the Sheet had not, and the two disagreed. Solo now registers itself through `combat.setStageProvider()` — a hook rather than an import, because `solo.js` already imports `combat.js` — so every surface reads the same spine. Every mid-play stage also gained an **ending**: table play shows *Finish for now* (End session), solo shows *Stop for tonight* mid-crisis or *Head home* once something is resolved, and the mid-session solo card carries it too. New `combat.howToPlay()` is a mode-aware three-beat card — Starting · Keeping it going · Finishing well — that names where you are right now and carries the buttons for each beat, reachable from both stage cards. | Not knowing how to stop is as disabling as not knowing how to start, and two contradictory "what now" answers are worse than none. | `npm test` — 490 passed, 0 failed; 10 new checks: the not-started stage offers no ending, an open session always can be finished, Crisis Mode makes the solo loop the spine (title and `#/solo` href), a solo session reports its step rather than the table lifecycle and can be stopped, Home renders one spine, and how-to-play is three beats written for the live mode. | v45 |
